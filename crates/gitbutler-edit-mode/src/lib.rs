...
use bstr::ByteSlice;
use git2::{build::CheckoutBuilder, Reference};
use gitbutler_branch::{
    get_integration_commiter, VirtualBranchesHandle, GITBUTLER_INTEGRATION_REFERENCE,
};
...
fn save_uncommited_files(ctx: &CommandContext) -> Result<()> {
    ...
    let signature = get_integration_commiter().context("Failed to get gitbutler signature")?;
